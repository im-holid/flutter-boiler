...
class AntiCarousel extends StatefulWidget {
  /// Creates slide show of Images and [Widget] with animation for sliding.
  const AntiCarousel({
    Key? key,
    required this.items,
    this.pagerWidth = 8.0,
    this.pagerHeight = 8.0,
    this.activePagerWidth = 8.0,
    this.activePagerHeight = 8.0,
    this.pagerMarginBottom,
  ...
  final double? activePagerWidth;
  final double? activePagerHeight;
  final double? pagerMarginBottom;

  // The position of dots [start, center, end]
  ...
  /// The [AntiCarousel] shows pagination on state true.
  ...
  /// The initial page to show when first creating the [AntiCarousel]. Defaults to 0.
  ...
  @override
  State<AntiCarousel> createState() => _AntiCarouselState();
}

class _AntiCarouselState extends State<AntiCarousel> with TickerProviderStateMixin {
  Timer? timer;
  ...
  void initState() {
    ...
    realPage = widget.enableInfiniteScroll ? realPage + widget.initialPage : widget.initialPage;
    pageController =
        PageController(viewportFraction: widget.viewportFraction as double, initialPage: realPage);
    timer = getPlayTimer();
  }
  ...
  Timer getPlayTimer() => Timer.periodic(widget.autoPlayInterval, (_) {
        if (widget.autoPlay && widget.items.length > 1) {
          pageController.nextPage(
              duration: widget.autoPlayAnimationDuration, curve: widget.autoPlayCurve);
        }
      });
        ...
        children: <Widget>[
          getPageWrapper(PageView.builder(
            ...
            onPageChanged: (int index) {
              ...
              currentPage =
                  _getRealIndex(index + widget.initialPage, realPage, widget.items.length);
          ...
          widget.hasPagination == true
              ? Positioned(
                  ...
                  bottom: 0,
                  child: Row(
                    mainAxisAlignment: widget.position!,
                    children: widget.map<Widget>(
                        widget.items,
                        (pagerIndex, url) => Container(
                              width: currentSlide == pagerIndex
                                  ? widget.activePagerWidth
                                  : widget.pagerWidth,
                              height: currentSlide == pagerIndex
                                  ? widget.activePagerHeight
                                  : widget.pagerHeight,
                              margin: EdgeInsets.only(
                                  top: 10,
                                  bottom: widget.pagerMarginBottom ?? 10.0,
                                  left: 2,
                                  right: 2),
